// pages/mid_autumn/index.js - 中秋诗词专题
...
Page({
  ...
  onLoad() {
    const topic = festivalData.getFestival('mid_autumn');
  ...
  /** 搜一搜优化：按专题上报标题与关键词（命中「中秋诗词」） */
  _setupSeo(topic) {
    const keywordParts = (topic.keywords || []).concat(['诗词', '古诗', '宋词', '国学', '超然古诗词']);
    seo.reportPageInfo({
  ...
  onShareAppMessage() {
    ...
    return {
      title: first ? `中秋诗词｜${first.title} · ${first.author}｜超然古诗词` : '中秋诗词 · 超然古诗词',
      path: '/pages/mid_autumn/index'
    };
  },
