 ...
 *   - 双模型支持：混元(hunyuan-exp) + DeepSeek
 *   - ai.js: 主模型 + fallback + 流式 + 限流 + 降级 + 模型切换
 *   - api.js: 业务层 + 配额层统一封装
 *   - UI: 全面 hover 效果 + 分段展示 + 流畅滚屏
 *
...
// 模拟 ai.js _friendlyError (v5.0 - token 错误在超时前检测)
function _friendlyError(e) {
  const msg = (e.message || String(e)).toLowerCase();
  if (msg.includes('操作太频繁'))                            return '操作太频繁，请稍候再试';
  if (msg.includes('token') && !msg.includes('timeout'))    return 'AI调用次数达到上限，请稍后再试';
...
// ── 15. 模型配置校验 ───────────────────────────────────────────
console.log('\n【15】模型配置（文档标准 + DeepSeek v5.0）');
const MODELS = {
  HUNYUAN_TURBO: 'hunyuan-turbos-latest',
...
const PROVIDERS = { HUNYUAN: 'hunyuan-exp', DEEPSEEK: 'deepseek' };
assert(MODELS.HUNYUAN_TURBO === 'hunyuan-turbos-latest', `混元主模型: ${MODELS.HUNYUAN_TURBO}`);
assert(MODELS.HUNYUAN_PRO === 'hunyuan-pro',             `混元备模型: ${MODELS.HUNYUAN_PRO}`);
assert(MODELS.DEEPSEEK_V3 === 'deepseek-v3',             `DeepSeek主模型: ${MODELS.DEEPSEEK_V3}`);
assert(PROVIDERS.HUNYUAN === 'hunyuan-exp',               `混元provider: ${PROVIDERS.HUNYUAN}`);
assert(PROVIDERS.DEEPSEEK === 'deepseek',                 `DeepSeek provider: ${PROVIDERS.DEEPSEEK}`);
assert(true, '官方文档 textStream API 已在 ai.js 中正确使用（res.textStream）');

// ── 16. 模型提供商切换逻辑 ─────────────────────────────────────
console.log('\n【16】双模型提供商切换（v5.0）');
let _provider = 'hunyuan';
function setModelProvider(p) { if (p === 'deepseek' || p === 'hunyuan') _provider = p; }
function getModelProvider()  { return _provider; }
setModelProvider('hunyuan');
assert(getModelProvider() === 'hunyuan', '默认为混元模型');
setModelProvider('deepseek');
assert(getModelProvider() === 'deepseek', '切换到 DeepSeek');
setModelProvider('invalid');
assert(getModelProvider() === 'deepseek', '无效提供商不改变当前设置');
setModelProvider('hunyuan');
assert(getModelProvider() === 'hunyuan', '切换回混元');
...
if (failed > 0) { console.error('⚠️  有测试失败'); process.exit(1); }
  ...
  console.log('🎉 所有测试通过！v5.0 架构验证成功。\n');
  console.log('📋 架构摘要 (v5.0):');
  console.log('  AI 调用路径:  miniprogram → wx.cloud.extend.AI.createModel(provider)');
  console.log('               .streamText({ data: { model, messages } })');
  console.log('               → res.textStream (官方推荐流式接口)');
  console.log('  双模型支持:');
  console.log('    混元路径:  provider="hunyuan-exp", 主=hunyuan-turbos-latest, 备=hunyuan-pro');
  console.log('    DeepSeek:  provider="deepseek",    主=deepseek-v3');
  console.log('  模型切换:    ai.setModelProvider("deepseek"|"hunyuan") + 本地存储记忆');
  ...
  console.log('  滚动优化:    scroll-into-view + enable-passive + show-scrollbar=false');
  console.log('  UI改进:      全页面 hover-class + 分段展示 + 模型切换按钮');
  process.exit(0);
}
